...
void VulkanEngine::init_vulkan() {
    ...
    auto inst_ret = builder.set_app_name("Example Vulkan Appliction")
...
void VulkanEngine::init_background_pipelines()
{
    ...
    if (!vkutil::load_shader_module("../../shaders/sky.comp.spv", _device, &skyShader)) {
        fmt::print("Error when building the night sky shader \n");
    }
    ...
    //change the shader module to only create the sky shader
    ...
    //add the 2 background effets into the array
...
void VulkanEngine::init_triangle_pipeline() {
    ...
    //we are not using descriptor sets or other systems yet, so no need to use any other than empty default
...
void GLTFMetallic_Roughness::build_pipelines(VulkanEngine* engine) {
    ...
    if (!vkutil::load_shader_module("../../shaders/mesh.vert.spv", engine->_device, &meshVertexShader)) {
        fmt::println("Error when buliding the triangle vertex shader module");
    }
...
void VulkanEngine::init_imgui() {
    ...
    // this initializes the core strucures of imgui
    ImGui::CreateContext();
...
void VulkanEngine::cleanup()
{
    if (_isInitialized) {
        ...
        //make sure the gpu has stopping doing its things
        vkDeviceWaitIdle(_device);
...
void VulkanEngine::draw()
{
    ...
    //> draw_3
    //naming it cmd for shorting writing
    VkCommandBuffer cmd = get_current_frame()._mainCommandBuffer;

    // now that we are sure that the commands finished executing, we can safely
    // reset the commmand buffer to begin recording again.
    ...
    //> draw_4
    //make the main image into writeable mode before rendering
    ...
    //draw imgui
    // set swapchain image layout to Attachment Optional so we can draw it
